...
public class AISubscriptionController {
    ...
    public ResponseEntity<ResponseDto<List<CustomAIRespDto>>> getSubscribedCustomAIsbyUserId() {
        try {
    ...
    public ResponseEntity<ResponseDto<List<AISubScriptionRespDto>>> getSubscriptionsbyUserId() {
        try {
            Long userId = CustomUserUtil.getCurrentUserId();
    ...
    public ResponseEntity<ResponseDto<AISubScriptionRespDto>> subscribe(@RequestBody AISubScriptionReqDto dto) {
        ...
        try {
            ...
            return new ResponseEntity<>(new ResponseDto<>(1, "AI 구독성공 -" + userLoginId, CustomDateUtil.toStringFormat(LocalDateTime.now()), aiSubScriptionRespDto), HttpStatus.OK);
